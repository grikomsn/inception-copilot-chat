...
export class ChatCompletionStreamParser {
  ...
  private parseBlock(block: string): ChatStreamEvent | undefined {
    ...
    if (json.error) throw new Error("Inception returned an error in the response stream");
...
export function validateStreamCompletion(finishReason: string | undefined): void {
  ...
  if (!finishReason) throw new Error("Inception response stream ended before a completion reason was received");
  if (finishReason === "length") throw new Error("Inception response reached its output token limit before completing");
  if (finishReason === "content_filter") throw new Error("Inception stopped the response because of its content filter");
  throw new Error(`Inception response ended with finish reason: ${finishReason}`);
}
...
function completeToolCall(tool: PendingToolCall): PendingToolCall {
  ...
  try {
    ...
    throw new Error(`Inception response stream ended with incomplete arguments for tool ${tool.name}`);
  }
